...
const CzechInvoiceGenerator = () => {
  ...
  const fetchFromAres = async (ico, target) => {
    ...
    try {
      // Zavoláme naši vlastní serverovou funkci
      const response = await fetch(`/.netlify/functions/ares?ico=${ico}`);

      if (!response.ok) {
        throw new Error('Chyba při komunikaci s naší proxy funkcí.');
      }
      ...
      const aresData = {
        ...
        address: `${subject.sidlo.ulice || ''} ${
          subject.sidlo.cisloOrientacni ? subject.sidlo.cisloOrientacni : ''
        }`.trim(),
              ...
              {invoice.items.map((item) => (
                <tr key={item.id} className="border-t">
                  ...
                  <td className="p-3 text-right">
                    {item.pricePerUnit.toFixed(2)} Kč
                  </td>
                  <td className="p-3 text-right">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    {supplier.logoUrl && (
                      <button
                        onClick={() => {
                          setSupplier({ ...supplier, logoUrl: '' }); // Smaže URL v objektu supplier
                          setLogoPreview(''); // Smaže dočasný náhled
                        }}
