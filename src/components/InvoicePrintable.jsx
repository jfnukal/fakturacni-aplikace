    ...
    const calculateTotals = () => {
      ...
      if (vatSettings.enabled) {
        invoice.items.forEach((item) => {
          ...
          // Sazba z položky → defaultRate z nastavení → rate (starý klíč) → 21 %
          // Podporuje obě pojmenování (defaultRate = nový systém, rate = starý systém)
          const itemVatRate = item.vatRate ?? vatSettings?.defaultRate ?? vatSettings?.rate ?? 21;
...
const paymentString = `SPD*1.0*ACC:${ibanAccount}*AM:${amount}*CC:CZK*MSG:Faktura ${invoice.number}*X-VS:${vs}`;
    ...
    // calculateTotals záměrně vynecháno z deps – je to inline funkce (nová reference každý render)
    // Závisí pouze na invoice, supplier a vatSettings, které jsou v deps
    ...
    return (
      ...
      >
              ...
              invoice.items.map((item, index) => (
                <tr key={item.id || index}>
                  ...
                  {vatSettings.enabled && (
                    ...
                    >
                      {item.vatRate ?? vatSettings?.defaultRate ?? vatSettings?.rate ?? 21}%
                    </td>
                  )}
                  <td
                    style={{
        ...
        {/* Razítko a podpis – zobrazí se jen pokud jsou vybrány na faktuře */}
        {((invoice.useStamp && supplier.stampUrl) ||
          (invoice.useSignature && supplier.signatureUrl)) && (
          <table
            style={{
              width: '100%',
              marginTop: '8mm',
          ...
          >
            <tbody>
              <tr>
                {/* Levá prázdná strana */}
                <td style={{ width: '55%' }} />
                {/* Pravá strana – razítko + podpis vedle sebe, zarovnáno vpravo */}
                <td
                  style={{
                    width: '45%',
