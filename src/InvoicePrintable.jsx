// Soubor: src/InvoicePrintable.jsx
import React from 'react';

const InvoicePrintable = React.forwardRef(({ invoice, supplier, vatSettings }, ref) => {
  ...
  return (
    <div ref={ref} className="p-8 bg-white text-black" style={{ width: '210mm', minHeight: '297mm', fontFamily: 'sans-serif', fontSize: '10pt' }}>
      ...
      <div className="grid grid-cols-2 gap-8 mb-10">
        <div className="space-y-1">
          <div><span className="font-semibold">Bankovní účet:</span> {supplier.bankAccount}</div>
          <div><span className="font-semibold">Variabilní symbol:</span> {invoice.number.replace(/-/g, '')}</div>
          <div><span className="font-semibold">Způsob platby:</span> {supplier.paymentMethod}</div>
        </div>
        <div className="space-y-1">
          <div><span className="font-semibold">Datum vystavení:</span> {invoice.issueDate}</div>
          <div><span className="font-semibold">Datum zdan. plnění:</span> {invoice.duzpDate}</div>
          <div><span className="font-semibold">Datum splatnosti:</span> {invoice.dueDate}</div>
        </div>
      </div>
        ...
        <tbody>
          {invoice.items.map((item) => (
            <tr key={item.id} className="border-b">
              ...
              <td className="p-2 text-right align-top">{Number(item.pricePerUnit).toFixed(2)} Kč</td>
              <td className="p-2 text-right align-top">{Number(item.totalPrice).toFixed(2)} Kč</td>
            </tr>
          ))}
        </tbody>
      </table>
