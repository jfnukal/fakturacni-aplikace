...
const ProductsPage = ({
  ...
  const isDirtyRef = useRef(false); // useRef, aby useEffect([initialProducts]) nepřepsal rozdrageované pořadí
  ...
  useEffect(() => {
    // Nepřepisuj lokální stav, pokud má uživatel neuložené změny (drag, editace…)
  ...
  const handleBulkSave = async () => {
    ...
    try {
      await batch.commit();
      // Resetujeme ref PŘED tím, než onSnapshot aktualizuje initialProducts.
      // Tím zajistíme, že useEffect([initialProducts]) smí resetovat stav,
      // ale až na správné (uložené) pořadí z Firestore.
      isDirtyRef.current = false;
      // Okamžitě aktualizujeme localProducts na uložené pozice, aby UI
      // nebylo závislé na rychlosti onSnapshot.
      setLocalProducts((current) =>
        current
          .filter((p) => p.status !== 'deleted')
          .map((p, idx) => ({ ...p, position: idx, status: 'pristine' }))
      );
      setIsDirty(false);
