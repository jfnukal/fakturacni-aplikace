...
useEffect(() => {
  ...
  const unsubscribe = onSnapshot(q, (querySnapshot) => {
    ...
    // Sekundární řazení podle čísla (číselně, ne textově)
    const sortedNotes = notes.sort((a, b) => {
      // Primárně podle data (sestupně - nejnovější nahoře)
      if (a.date !== b.date) {
        return b.date.localeCompare(a.date);
      }
      
      // Sekundárně podle čísla - extrahujeme číselnou část
      const getNumber = (numStr) => {
        const match = numStr.match(/(\d+)$/);
        return match ? parseInt(match[1], 10) : 0;
      };
      
      return getNumber(b.number) - getNumber(a.number);
    });
  ...
  const calculateTotals = (items) => {
    if (!items)
      return { totalWithoutVat: 0, totalWithVat: 0, vatBreakdown: {} };
  ...
  const getNextBusinessDay = () => {
    ...
    if (dayOfWeek === 6) {
      // Sobota - přidej 2 dny (pondělí)
      nextDay.setDate(nextDay.getDate() + 2);
    } else if (dayOfWeek === 0) {
      // Neděle - přidej 1 den (pondělí)
  ...
  const getNewDeliveryNote = () => {
    ...
    // Výchozí sazba DPH: z nastavení (defaultRate = nový systém, rate = starý systém)
    const defaultVatRate = vatSettings?.defaultRate ?? vatSettings?.rate ?? 21;
    ...
    return {
      ...
      items: products.map((p) => ({
        ...
        price: p.price,
        quantity: 0,
        // DPH: z produktu → globální výchozí → 21 %
        vatRate: p.defaultVatRate ?? defaultVatRate,
      })),
      userId: currentUser.uid,
      showPrices: true,
      // Snapshot DPH platný v momentě vytvoření dokladu (pro správný tisk i po změně sazby)
      vatSnapshot: {
        enabled: vatSettings.enabled,
        // Uložíme jako pole pro zpětnou kompatibilitu s existujícími DL
        rates: [defaultVatRate],
      },
    };
  };
    ...
    setCurrentDeliveryNote({
      ...
      items: products.map((p) => {
        ...
        if (existingItem) {
          ...
          return {
            ...existingItem,
            vatRate: existingItem.vatRate ?? defaultVatRate,
          };
        } else {
  ...
  const saveDeliveryNoteFlow = async (noteData) => {
    ...
    const noteToSave = {
      ...noteData,
      items: noteData.items.filter((item) => item.quantity > 0),
    };
    const { totalWithoutVat, totalWithVat, vatBreakdown } = calculateTotals(
      noteToSave.items
    );
            ...
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr className="hidden md:table-row">
                  <th className="text-left p-4 font-medium">
                    {t('delivery_notes_page.table.number')}
                  </th>
                  <th className="text-left p-4 font-medium">
                    {t('delivery_notes_page.table.customer')}
                  </th>
                  <th className="text-left p-4 font-medium">
                    {t('delivery_notes_page.table.issue_date')}
                  </th>
                  <th className="text-right p-4 font-medium">
                    {t('delivery_notes_page.table.amount')}
                  </th>
                  <th className="text-center p-4 font-medium">
                    {t('delivery_notes_page.table.actions')}
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200">
                {deliveryNotes.map((note) => {
                  ...
                  const { totalWithoutVat, totalWithVat } = calculateTotals(
                    note.items
                  );
                  return (
                    <tr
                        ...
                        <div className="space-y-2">
                          <div className="flex justify-between items-start">
                            <div>
                              ...
                              <div className="text-gray-600 text-sm">
                                {note.customer?.name || 'Bez odběratele'}
                              </div>
                          ...
                          >
                            <button
                              ...
                              title={
                                !note.customer
                                  ? 'Dodací list nemá vybraného odběratele'
                                  : t('common.print')
                              }
                            >
                              <Printer size={16} />
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-2xl font-bold">
              {editingNote
                ? t('delivery_notes_page.edit_title')
                : t('delivery_notes_page.new_title')}
            </h2>
          </div>
          <div className="bg-gray-50 rounded-lg p-6 space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 border-b pb-6">
              <div>
                <label className="block text-sm font-medium mb-2">
                  {t('delivery_notes_page.form.number')}
                </label>
                <input
                  type="text"
                  value={currentDeliveryNote.number}
                  onChange={(e) =>
                    setCurrentDeliveryNote({
                      ...currentDeliveryNote,
                  ...
                  className="w-full p-3 border rounded-lg text-base focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>
              <div>
                <label className="block text-sm font-medium mb-2">
                  {t('delivery_notes_page.form.issue_date')}
                </label>
                <input
                  type="date"
            ...
            <div>
              <label className="block text-base font-medium mb-3">
                {t('delivery_notes_page.form.customer')}
              </label>
              <select
                defaultValue={currentDeliveryNote.customer?.id || ''}
                onChange={(e) =>
                  setCurrentDeliveryNote({
                    ...currentDeliveryNote,
                    customer:
                      savedCustomers.find((c) => c.id === e.target.value) ||
                      null,
                  })
                }
                className="w-full p-3 border rounded-lg text-base focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
                <option value="">
                  {t('delivery_notes_page.form.select_customer')}
                </option>
              ...
              <div className="space-y-3">
                <div className="hidden md:grid grid-cols-12 gap-3 text-sm font-medium text-gray-600 px-3">
                  ...
                  <div className="col-span-3 text-center">
                    {t('products_page.header.unit')} / Množství
                  </div>
                </div>
                  ...
                  >
                    ...
                    <div className={`col-span-2 md:col-span-3`}>
                      ...
                      <div className="flex items-center">
                        <span className="pr-2 text-gray-500 text-sm hidden md:inline">
                          {item.unit}
                        </span>
                        <input
                          type="number"
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pt-4 border-t">
              <label className="flex items-center gap-3">
                <input
                  type="checkbox"
                  checked={currentDeliveryNote.showPrices}
                  onChange={(e) =>
                    setCurrentDeliveryNote({
                      ...currentDeliveryNote,
                ...
                <span className="text-sm sm:text-base">
                  {t('delivery_notes_page.form.show_prices')}
                </span>
              </label>
              <div className="text-right w-full sm:w-auto">
                <h3 className="text-base sm:text-lg font-bold">
                  {t('delivery_notes_page.form.total_without_vat')}{' '}
                  {calculateTotals(
                    currentDeliveryNote.items
                  ).totalWithoutVat.toFixed(2)}{' '}
                  Kč
                </h3>
                {currentDeliveryNote.vatSnapshot?.enabled && (
                  <h4 className="text-sm sm:text-md text-gray-600">
                    {t('delivery_notes_page.form.total_with_vat')}{' '}
                    {calculateTotals(
                      currentDeliveryNote.items
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <button
              onClick={handleSave}
              className="flex items-center justify-center gap-2 px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium text-base transition-colors"
            >
              <Save size={18} />
              {editingNote
                ? t('common.save_changes')
                : t('delivery_notes_page.save')}
            </button>
            <button
            ...
            <button
              onClick={() => setCurrentView('list')}
              className="px-6 py-3 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 font-medium text-base transition-colors"
            >
              {t('common.cancel')}
            </button>
          </div>
