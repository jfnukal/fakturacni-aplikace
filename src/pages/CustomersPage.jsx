...
const CustomersPage = ({ savedCustomers, creationRequest, setCreationRequest, selectCustomerForNewInvoice }) => {
  ...
  const handleAresLookup = async (ico) => {
    ...
    if (aresData) {
      // Zachováme pole která ARES nezná (notes), zbytek přepíšeme
  ...
  // TOTO JE JEDINÁ ZMĚNA, KTEROU JSME OPROTI PŮVODNÍMU SOUBORU UDĚLALI
              ...
              <thead className="hidden md:table-header-group bg-gray-50">
                <tr>
                  <th className="text-left p-4 font-medium">
                    {t('customers_page.table.name')}
                  </th>
                  <th className="text-left p-4 font-medium">
                    {t('customers_page.table.ico')}
                  </th>
                  <th className="text-left p-4 font-medium">
                    {t('customers_page.table.city')}
                  </th>
                  <th className="text-center p-4 font-medium">
                    {t('customers_page.table.actions')}
                  </th>
                </tr>
              </thead>
                ...
                {(savedCustomers || []).map((customer) => (
                  <tr key={customer.id} className="block md:table-row p-4">
                    <td
                      ...
                      data-label={`${t('customers_page.table.name')}: `}
                    >
                      {customer.name}
                    </td>
                    <td
                      className="block md:table-cell md:p-4"
                      data-label={`${t('customers_page.table.ico')}: `}
                    >
                      {customer.ico}
                    </td>
                    <td
                      className="block md:table-cell md:p-4"
                      data-label={`${t('customers_page.table.city')}: `}
                    >
                      {customer.city}
                    </td>
                    <td className="block md:table-cell md:p-4">
                      <div className="flex gap-2 justify-end md:justify-center mt-2 md:mt-0">
                        <button
                          ...
                          title={t('customers_page.action.create_invoice')}
                        >
                          <FileText size={20} />
                        </button>
                        <button
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-2xl font-bold">
              {editingCustomer.id
                ? t('customers_page.edit_title')
                : t('customers_page.new_title')}
            </h2>
          </div>
          <div className="bg-gray-50 rounded-lg p-6 grid md:grid-cols-2 gap-4">
            <input
              type="text"
              placeholder={t('customers_page.form.company_name')}
            ...
            <input
              type="text"
              placeholder={t('customers_page.form.address')}
            ...
            <input
              type="text"
              placeholder={t('customers_page.form.zip')}
            ...
            <input
              type="text"
              placeholder={t('customers_page.form.city')}
            ...
            <div className="flex items-end gap-2">
              <div className="flex-grow">
                <input
                  type="text"
                  placeholder={t('customers_page.form.ico')}
              ...
              <button
                type="button"
                onClick={() => handleAresLookup(editingCustomer.ico)}
                disabled={aresLoading}
                className="px-4 py-2 bg-gray-200 rounded hover:bg-gray-300 text-sm h-10 disabled:opacity-50"
              >
                {aresLoading ? '...' : t('customers_page.form.ares')}
              </button>
            </div>
            <input
              type="text"
              placeholder={t('customers_page.form.dic')}
            ...
            <textarea
              ...
              placeholder={t('customers_page.form.notes')}
              rows="3"
              className="md:col-span-2 w-full p-2 border rounded"
            />
          </div>
